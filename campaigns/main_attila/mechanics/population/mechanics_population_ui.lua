...
function Population_UI_Get_Growth_Factor(growth)
	...
	return "\nПрогнозоване зростання: "..color.."("..symbol..growth.."%)".."[[/rgba]]";
end
...
function OnComponentMouseOn_Population_UI(context)
		...
		if unit_population_table then
			...
			if unit_class_string then
				...
				if not LAST_CHARACTER_SELECTED:faction():is_horde() then
					...
					newToolTip = "Доступна чисельність "..unit_class_string.." в регіоні "..REGIONS_NAMES_LOCALISATION[region_name]..": "..tostring(POPULATION_REGIONS_MANPOWER[region_name][unit_class]).."\nНеобхідна кількість людей: "..unit_population_table[1];
	...
	elseif string.find(context.string, "QueuedLandUnit") then
		...
		if unit_population_table then
			...
			if unit_class_string then
				local newToolTip = "ЛКМ - видалити цей загін з черги.";
				
				if not LAST_CHARACTER_SELECTED:faction():is_horde() then
					local region_name = LAST_CHARACTER_SELECTED:region():name();
					newToolTip = "ЛКМ - видалити цей загін з черги.\n\nЦе поверне "..unit_population_table[1].." "..unit_class_string.." в регіон "..REGIONS_NAMES_LOCALISATION[region_name]..".";
				end
...
function OnComponentLClickUp_Population_UI(context)
	...
	elseif context.string == "button_tick" then
		if UIComponent(context.component):GetTooltipText() == "Підтвердити" and button_disband_pressed == true then
		...
		elseif UIComponent(context.component):GetTooltipText() == "Скасувати" and button_disband_pressed == true then
			button_disband_pressed = false;
		end
...
function Change_Tooltip_Population_UI(key, class, own_region)
	...
	elseif class ~= 0 then
		...
		local class_population = "Чисельність: "..tostring(POPULATION_REGIONS_POPULATIONS[key][class].."\n\n");
		local projected_growth = "\nПрогнозована швидкість приросту: "..Population_UI_Get_Growth(key, class);
		...
		if math.ceil(POPULATION_REGIONS_POPULATIONS[key][class] * POPULATION_REGIONS_GROWTH_RATES[key][class]) < 0 then
			projected_growth_num = "\nПрогнозоване зростання: [[rgba:255:0:0:150]]"..tostring(math.ceil(POPULATION_REGIONS_POPULATIONS[key][class] * POPULATION_REGIONS_GROWTH_RATES[key][class])).."[[/rgba]] "..class_string;
		elseif math.ceil(POPULATION_REGIONS_POPULATIONS[key][class] * POPULATION_REGIONS_GROWTH_RATES[key][class]) == 0 then
			if POPULATION_REGIONS_GROWTH_RATES[key][class] == 0 or POPULATION_REGIONS_GROWTH_RATES[key][class] < 0 then
				projected_growth_num = "\nПрогнозоване зростання: [[rgba:255:255:0:150]]0[[/rgba]] "..class_string;
			elseif POPULATION_REGIONS_GROWTH_RATES[key][class] > 0 then
				projected_growth_num = "\nПрогнозоване зростання: [[rgba:8:201:27:150]]+1[[/rgba]] "..POPULATIONS_CLASSES_STRINGS[class];
			end
		else
			projected_growth_num = "\nПрогнозоване зростання: [[rgba:8:201:27:150]]+"..tostring(math.ceil(POPULATION_REGIONS_POPULATIONS[key][class] * POPULATION_REGIONS_GROWTH_RATES[key][class])).."[[/rgba]] "..class_string;
		end
			...
			for i = 1, #first_split do
				...
				if second_split[1] == "faction_trait_"..tostring(class) then
					...
					if tonumber(second_split[2]) < 0 then
						faction_trait_growth = "Приріст від особливостей фракції: ".."[[rgba:255:0:0:150]]("..second_split[2].."%)[[/rgba]]\n";
					elseif tonumber(second_split[2]) == 0 then
						-- Should never be the case.
						faction_trait_growth = "Приріст від особливостей фракції: ".."[[rgba:255:255:0:150]](0%)[[/rgba]]\n";
					else
						faction_trait_growth = "Приріст від особливостей фракції: ".."[[rgba:8:201:27:150]](+"..second_split[2].."%)[[/rgba]]\n";
					end
				elseif second_split[1] == "technologies_"..tostring(class) then
					second_split[2] = Round_Number_Text(second_split[2]);
	
					if tonumber(second_split[2]) < 0 then
						technology_growth = "Приріст від розвитку технологій: ".."[[rgba:255:0:0:150]]("..second_split[2].."%)[[/rgba]]\n";
					elseif tonumber(second_split[2]) == 0 then
						-- Should never be the case.
						technology_growth = "Приріст від розвитку технологій: ".."[[rgba:255:255:0:150]](0%)[[/rgba]]\n";
					else
						technology_growth = "Приріст від розвитку технологій: ".."[[rgba:8:201:27:150]](+"..second_split[2].."%)[[/rgba]]\n";
					end
				elseif second_split[1] == "buildings_"..tostring(class) then
					second_split[2] = Round_Number_Text(second_split[2]);
					building_growth = "Приріст від будівель: ".."[[rgba:8:201:27:150]](+"..second_split[2].."%)[[/rgba]]\n";
				elseif second_split[1] == "capital_bonus_"..tostring(class) then
					--second_split[2] = Round_Number_Text(second_split[2]);
					capital_bonus = "Столиця фракції: ".."[[rgba:8:201:27:150]](+"..second_split[2].."%)[[/rgba]]\n";
				elseif second_split[1] == "imperial_decree_"..tostring(class) then
					--second_split[2] = Round_Number_Text(second_split[2]);
					imperial_decree = "Імператорські укази: ".."[[rgba:8:201:27:150]](+"..second_split[2].."%)[[/rgba]]\n";
				elseif second_split[1] == "hard_cap_exceeded" then
					second_split[2] = Round_Number_Text(second_split[2]);
					cap_exceeded = "Перевищено жорсткий ліміт: ".."[[rgba:255:0:0:150]](-"..second_split[2].."% of growth)[[/rgba]]\n";
				elseif second_split[1] == "soft_cap_exceeded" then
					second_split[2] = Round_Number_Text(second_split[2]);
					cap_exceeded = "Перевищено м'який ліміт: ".."[[rgba:255:0:0:150]](-"..second_split[2].."% of growth)[[/rgba]]\n";
				elseif second_split[1] == "black_death" then
					second_split[2] = Round_Number_Text(second_split[2]);
					cap_exceeded = "Спалах чуми: ".."[[rgba:255:0:0:150]](-"..second_split[2].."%)[[/rgba]]\n";
				elseif second_split[1] == "under_siege" then
					second_split[2] = Round_Number_Text(second_split[2]);
					under_siege = "В облозі: ".."[[rgba:255:0:0:150]](-"..second_split[2].."%)[[/rgba]]\n";
				elseif second_split[1] == "food_shortage" then
					second_split[2] = Round_Number_Text(second_split[2]);
					food_shortage = "Нестача їжі: ".."[[rgba:255:0:0:150]](-"..second_split[2].."%)[[/rgba]]\n";
				elseif second_split[1] == "region_raided" then
					second_split[2] = Round_Number_Text(second_split[2]);
					region_raided = "Нальоти в регіоні: ".."[[rgba:255:0:0:150]](-"..second_split[2].."%)[[/rgba]]\n";
				end
			end
		end
